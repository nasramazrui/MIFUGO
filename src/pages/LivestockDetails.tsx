          ...
          <div className="flex border-b border-emerald-50 overflow-x-auto no-scrollbar bg-white sticky top-0 z-30">
            {[
              ...
              <button
                ...
                className={`flex-1 flex items-center justify-center px-4 sm:px-6 py-4 sm:py-5 text-xs sm:text-sm font-bold whitespace-nowrap border-b-4 transition-all ${
                  activeTab === tab.id 
                    ? 'border-emerald-500 text-emerald-600 bg-emerald-50/30' 
                    : 'border-transparent text-gray-400 hover:text-emerald-500'
                }`}
              >
                <tab.icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 ${activeTab === tab.id ? 'animate-pulse' : ''}`} />
                {tab.label}
              </button>
            ))}
          </div>
